    ...
    public class BackTest2
    {
     ...
     //   public DoWorkStatus{} get; set;
        ...
        public TestModeEnum TestMode
        {
            ...
            {
                _testMode = value;
                //if(_testMode == TestModeEnum.Real)
                //    CreateSeries();
            }
        }
        ...
        private IList<TickerSlot> _ticerSlots = new List<TickerSlot>(); 
        ...
        public void Init(ITradeContext tx)
        {
            ...
            ExecutionBarMode = ExecutionBarModeEnum.Compr;
        ...
        private void DoWork()
        {
            ...
            while (_doWorkStatus)
            {
                ...
                else if (TestMode == TestModeEnum.Real)
                {
                    ...
                    if (bar != null)
                    {
                        _flagNewDay = (_lastBar != null && bar.DT.DateToInt() > _lastBar.DT.DateToInt()) ||
                                        _lastBar == null;
                        ...
                        switch (ExecutionBarMode)
                        {
                            case ExecutionBarModeEnum.TypicalPrice:
                                _tradeContext.ExecuteTick(bar.DT, _tickerSlot.Ticker.Key, bar.MedianPrice,
                                    bar.TypicalPrice, bar.TypicalPrice);
                                break;
                            case ExecutionBarModeEnum.MedianPrice:
                                _tradeContext.ExecuteTick(bar.DT, _tickerSlot.Ticker.Key, bar.MedianPrice,
                                    bar.MedianPrice, bar.MedianPrice);
                                break;
                            case ExecutionBarModeEnum.OC:
                                _tradeContext.ExecuteTick(bar.DT, _tickerSlot.Ticker.Key, bar.MedianPrice, bar.Open,
                                    bar.Open);
                                _tradeContext.ExecuteTick(bar.DT, _tickerSlot.Ticker.Key, bar.MedianPrice, bar.Close,
                                    bar.Close);
                                break;
                        ...
                        _tradeContext.Tickers.UpdateAsyncSeries(_tickerSlot.Ticker.Key, bar);
        ...
        private TestModeEnum _testMode;
