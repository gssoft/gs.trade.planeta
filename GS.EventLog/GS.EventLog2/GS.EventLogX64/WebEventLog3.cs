...
namespace GS.EventLog
{
    // using DbEventLog = GS.EventLog.DataBase1.Model.DbEventLog;
    // using EventLogItemDto = GS.EventLog.DataBase1.Model.EventLogItemDto;
    public class WebEventLog3 : Evl, IEventLog
    {
        ...
        public int ErrorPostTimeOut { get; set; }

        public string ApiTokenStr => "/oauth/token";
        ...
        private void AddItemSync(EventLogItemDto evli)
        {
            ...
            try
            {
                using (HttpResponseMessage response = _client.PostAsJsonAsync(ApiEventLogItems, evli).Result)
                {
                    ...
                    {
                        var evlUri = response.Headers.Location;
                        // Console.WriteLine("AddItemSync(): Success: {0}, Phrase: {1}", response.StatusCode, response.ReasonPhrase);

                        var oldEnable = IsEnabled; 
                        IsEnabled = true;
                        if(oldEnable != IsEnabled)
                            Parent.Evlm2(EvlResult.SUCCESS, EvlSubject.TECHNOLOGY, FullCode, Code,
                    ...
                    {   // (int)response.StatusCode, response.ReasonPhrase);
                        ...
                        Parent.Evlm2(EvlResult.FATAL, EvlSubject.TECHNOLOGY, FullCode, System.Reflection.MethodBase.GetCurrentMethod().Name,
                            "Error in PostAsJson", "ResponseStatus: " + response.StatusCode + " " + response.ReasonPhrase, "");

                        StartRepairTask(evli, "PostItem Error:");
                    }
                }
            }
            catch (Exception e)
            {
                ...
                StartRepairTask(evli, "PostItem Exception:");
            }
            //return null;
            //return -1;
        ...
        private void StartRepairTask(EventLogItemDto evli, string logmessage)
        {
            Task.Factory.StartNew(i =>
            {
                var ei = (EventLogItemDto)i;
                Parent.Evlm2(EvlResult.FATAL, EvlSubject.TECHNOLOGY, FullCode, "AddItemsync()", logmessage, "Start Repair Task", "");
                //Console.WriteLine("----------------------------------------------------------------");
                //Console.Write(FullCode + ": Start RepairTask");
                Thread.Sleep(TimeSpan.FromSeconds(ErrorTimeOut));
                Parent.Evlm2(EvlResult.FATAL, EvlSubject.TECHNOLOGY, FullCode, "AddItemsync()", logmessage,"Try to RePost Item", "");
                //Console.Write(FullCode + ": RepairTask: Start to AddItemSync");
        ...
        public int ErrorTimeOut {
            get { return ErrorPostTimeOut > 0 ? ErrorPostTimeOut : 60; }
        }
                            ...
                            string objects)
        {
            //if (WebDbEventLog == null)
            //    InitEventLog();
            if (EventLogKey.HasNoValue())
            {
                IsEnabled = false;
                SendExceptionMessage3(FullCode, GetType().FullName,
                                        System.Reflection.MethodBase.GetCurrentMethod().Name, "", 
                                        new NullReferenceException("EventLogKey is NUll"));
                return;
            }
            if (!IsEnabled)
                return;
