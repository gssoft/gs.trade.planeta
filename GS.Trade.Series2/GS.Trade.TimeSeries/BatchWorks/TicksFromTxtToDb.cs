...
{
    public class TicksFromTxtToDb  : Element1<string>, IBatchWorkItem
    {
        ...
        private void AddTicks()
        {
            try
            {
                ...
                using (var context = new FortsTicksContext(ConnectionString))
                {
            ...
            {
                SendException(FullCode, "AddTick()", ex);
            }
        }
        ...
        private void UpdateTicksStatAll()
        {
            try
            {
                using (var db = new FortsTicksContext(ConnectionString))
                {
