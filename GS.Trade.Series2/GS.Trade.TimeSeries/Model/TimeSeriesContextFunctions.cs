...
namespace GS.Trade.TimeSeries.Model
{
    ...
    public partial class TimeSeriesContext
    {
        ...
        // 2020.06.13
        public IQueryable<BarDto> GetBarsDto(string ticker, int? timeint)
        {
            if (ticker.HasNoValue()) return null;
            var series = GetBarSeries(ticker, timeint);
            var bs = (Bars
                .Where(b => b.BarSeriesId == series.Id)
                .AsNoTracking()
                .OrderBy(b => b.Id)
                .Select(b => new BarDto
                {
