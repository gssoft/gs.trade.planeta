...
namespace GS.Trade.Strategies
{
    ...
    public class X105 : Strategy
    {
      ...
      //  private BarSeries _bars;
        ...
        private double _lastLimitPrice;
        ...
        public X105(ITradeContext tx, string name, string code, Ticker ticker, uint timeInt)
        ...
        public override void Main()
        {
         //   if (TimePlan.IsTimeToRest) return;
            ...
            {
                    _currentLimitPrice = _xTrend.GetPrice2(+1, 01, EntryMode, out _comment);

                    if (!_currentLimitPrice.Equals(0.0))
                    {
                        if (_lastLimitBuyPrice.Equals(_currentLimitPrice)) return;
                        _lastLimitBuyPrice = _currentLimitPrice;

                        var stopPrice = _currentLimitPrice - KAtrStop * Ma1KAtr * _xAtr.LastAtrCompleted;

                        SetOrder(OrderOperationEnum.Buy, Contracts, _currentLimitPrice, stopPrice);
                        return;
                    }
                    _currentLimitPrice = _xTrend.GetPrice2(-1, 01, EntryMode, out _comment);
                    ...
                    if (! _currentLimitPrice.Equals(0.0))
                    {
                        ...
                        SetOrder(OrderOperationEnum.Sell, Contracts, _currentLimitPrice, stopPrice);
                        return;
                    }
        ...
        private void SetOrder(OrderOperationEnum operation, long contracts, double entryPrice, double stopPrice)
        {

            OrderOperationEnum flipOperation = OrderOperationEnum.Unknown;
            int stopDirection = 0;
            ...
            else if (operation == OrderOperationEnum.Sell)
            {
                if (_lastLimitSellPrice.Equals(_currentLimitPrice)) return;
                _lastLimitSellPrice = _currentLimitPrice;
            ...
            //var contract = Position.IsNeutral ? Contracts : Contracts * 2;
