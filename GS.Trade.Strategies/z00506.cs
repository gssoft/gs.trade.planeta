...
namespace GS.Trade.Strategies
{
    public class Z00506 : Z005
    {
        ...
        protected override void Prefix()
        {
            ...
            if (IsUnSafeContracts)
            {
                if (!PositionStop.IsValid)
                {
                    if (Position.IsShort)
                    {
                        if (XTrend.IsDown)
                            PositionStop.SetValidValue((float)XTrend.High2);
                    }
                    else if (Position.IsLong)
                    {
                        if (XTrend.IsUp)
                            PositionStop.SetValidValue((float)XTrend.Low2);
                    }
                }
                else
                {
                    if (Position.IsShort)
                    {
                        if (XTrend.IsDown)
                        {
                            var max = (float)XTrend.High2;
                           // if (max.CompareTo(PositionStop) >= 0)
                            PositionStop.SetIfLessThan(max);
                            //if( PositionStop.IsLessThan(max) )
                            //    PositionStop.Value = max;
                        }
                    }
                    if (Position.IsLong)
                    {
                        if (XTrend.IsUp)
                        {
                            var min = (float)XTrend.Low2;
                            PositionStop.SetIfGreaterThan(min);
                            //if (PositionStop.IsGreaterThan(min))
                            //    PositionStop.Value = min;
                        }
                    }
                }
        ...
        protected override int ShortExit2()
        {
            ...
            */
            if (PositionStop.IsLessThan((float)XTrend.Ma))
            {
                if (XTrend.GetPrice5(+1, ExitPriceId, out _currentLimitPrice, out _comment))
                {
                    _tryEnterMode = -1;
        ...
        protected override int LongExit2()
        {
            ...
            */
            if (PositionStop.IsGreaterThan((float)XTrend.Ma))
            {
                if (XTrend.GetPrice5(-1, ExitPriceId, out _currentLimitPrice, out _comment))
                {
                    _tryEnterMode = -1;
