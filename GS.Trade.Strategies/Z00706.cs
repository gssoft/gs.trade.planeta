...
namespace GS.Trade.Strategies
{
    public class Z00706 : Z007
    {
            ...
            if ((Position.IsOpened && Position.Quantity < Contracts))
            {
                switch (Mode)
                {
                    ...
                    case 2:
                        ...
                        else if (XTrend.Trend55.Value < 0)
                        {
                            if (Position.IsLong)
                            {
                                if (LongEntry(0))
        ...
        private void ChangeMode123()
        {
            ...
            if (Position.IsOpened )
            {
                if ( XTrend.Trend55.Value > 0 && Position.IsLong ||
                     XTrend.Trend55.Value < 0 && Position.IsShort
                   // (
                   //     Position.IsLong &&
