    ...
    public class Z0073521 : Z007
    {
        ...
        protected override void CalcPosMinMax()
        {
            ...
            if (Position.IsShort)
            {
                ...
                else if (XTrend.IsUp)
                {
                    if (PositionMin.SetIfGreaterThan((float) XTrend.Low2))
                        PositionStop.Value = PositionMin.GetValidValue + StopValue;
            ...
            else if (Position.IsLong)
            {
                ...
                {
                    if( PositionMax.SetIfLessThan((float)XTrend.High2))
                        PositionStop.Value = PositionMax.GetValidValue - StopValue; 
        ...
        private bool ShortEntry(int signal)
        {
            ...
            if (LastTradeMaValue.IsEquals(XTrend.Ma))
                return false;
        ...
        private bool LongEntry(int signal)
        {
           ...
           // int entryMode = 0;
            ...
            if (LastTradeMaValue.IsEquals(XTrend.Ma))
                return false;
            //  LastTradeMaValue Update in PositionIsChangedEventHandler2
            //  LastTradeMaValue = XTrend.Ma;
                ...
                )
            {
                ...
                if (XTrend.GetPrice6(+1, EntryPriceId, LastSignalActivated ,out _currentLimitPrice, out _comment))
                {
                    ...
                    TradeContext.Evlm(EvlResult.INFO, EvlSubject.TRADING, StrategyTickerString,
                        "Signal: " +  LastSignalActivated,
                        operStr,descrStr,"");
            ...
            if (Position.IsOpened )
            {
                if (Position.Quantity < Contracts)
                {
                    ...
                    if (IsPosPositive)
                    {
                        ...
                        Mode = IsPosAbsMaxReached && _swingCountEntry > 1
                            ? 5
                            : 3;
                        // Mode = 3;
                        //Mode = 4;
