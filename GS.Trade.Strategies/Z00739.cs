...
namespace GS.Trade.Strategies
{
    public class Z00739 : Z007
    {
        ...
        protected override int ShortExit2()
        {
            ...
            {
                if (XTrend.GetPrice6(+1, ExitPriceId, LastSignalActivated, out _currentLimitPrice, out _comment))
                {
                    _tryEnterMode = -1;
                    LastExitMode = _mode;
            ...
            if (Position.IsOpened )
           {
                if (Position.Quantity < Contracts)
                {
                    ...
                    else
                    {
                        if ( // TrendMode == 1 &&
                            ...
                            Mode = 2;
                        else if (IsMode2Delayed && (Mode == 3 || Mode == 2))
                        {
                            TrEntryEnabled.Reset();
                            _swingCountEntry = 0;
                            Mode = 8;
                        }
                        else if (IsMode2Delayed && Mode == 8)
                        {
                            if (TrEntryEnabled.Value &&
                ...
                else if (Position.Quantity >= Contracts)
                {
                    if (
                        ...
                        Mode = 3;
                    else if (IsMode3Delayed && (Mode == 3 || Mode == 2))
                    {
                        TrEntryEnabled.Reset();
                        _swingCountEntry = 0;
                        Mode = 8;
                    }
                    else if (IsMode3Delayed && Mode == 8)
                    {
                        if (TrEntryEnabled.Value &&
                            ((Position.IsLong && TakeShort(25)) || (Position.IsShort && TakeLong(25)))
                            )
