...
{
    public class Z00736Z1 : Z007
    {
        public double SafePosRatio { get; set; }
        public int SafePos => (int) (SafePosRatio*Contracts);
        ...
        private bool ShortEntry(int signal)
        {
            ...
            //        return false;
            //} 
            //if (!TrEntryEnabled.Value && Position.Quantity <= SafePos)
            // if ((!TrEntryEnabled.Value && (Mode == 2 || Mode==8)) || _swingCountEntry < 1)
            if (!TrEntryEnabled.Value && _swingCountEntry < 1)
                return false;
        ...
        private void ChangeMode123()
        {
            ...
            if (Position.IsOpened )
            {
                ...
                //}
                    if (Position.Quantity < Contracts)
                    {
                        if (Position.Quantity <= SafePos)
                            Mode = 2; // 19/19
                        // else Mode = 3; // 19/223
                        else
                        {
                            if(IsPosPositive) Mode = 2;  // 19/19
                            else Mode = 3; // 19/223
                        }
                    }
