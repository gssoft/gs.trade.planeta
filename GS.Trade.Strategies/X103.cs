...
namespace GS.Trade.Strategies
{
  ...
  //      private BarSeries _bars;
        ...
        public X103(ITradeContext tx, string name, string code, Ticker ticker, uint timeInt)
        ...
        public override void Main()
        {
          //  if (TimePlan.IsTimeToRest) return;
            ...
            else // Position == 0
            {
                ...
                */
                if (_xTrend.IsUp /* && !_waitFlat */&& !_xTrend.IsDown2)
                {
                    _currentLimitPrice = _xma018.GetPrice(+1, EntryMode, out _comment);
                ...
                else if (_xTrend.IsDown /* && !_waitFlat */  && !_xTrend.IsUp2)
                {
                    _currentLimitPrice = _xma018.GetPrice(-1, EntryMode, out _comment);
                ...
                if (_xTrend.IsUp2 && _xTrend.IsDown
                    && _xTrend.Low.CompareTo(_xTrend.Low2 + _kAtrZone1 * _xAtr.LastAtrCompleted) < 0  )  //  First was <=
                {
                ...
                if (_xTrend.IsDown2 && _xTrend.IsUp
                    && _xTrend.High.CompareTo(_xTrend.High2 - _kAtrZone1 * _xAtr.LastAtrCompleted) > 0) // At First was >=
                {
