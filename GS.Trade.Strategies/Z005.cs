...
{
    public partial class Z005 : Strategy
    {
        [XmlIgnore]
        ...
        {
            get { return    PositionMax.IsValid && PositionMin.IsValid
        ...
        public Z005(ITradeContext tx, string name, string code, Ticker ticker, uint timeInt)
        ...
        public override void Main()
        {
            ...
            if (Position.IsShort)
            {
                ...
                if (XTrend.IsDown)
                {
                    if(PositionMax.SetValidValue((float) XTrend.High2))
                        PositionStop.Value = PositionMax.GetValidValue;
                }
                else if (XTrend.IsUp)
                    PositionMin.SetValidValue((float) XTrend.Low2);
            ...
            else if (Position.IsLong)
            {
                ...
                if (XTrend.IsUp)
                {
                    if( PositionMin.SetValidValue((float) XTrend.Low2))
                        PositionStop.Value = PositionMin.GetValidValue;
                }
                else if (XTrend.IsDown)
                    PositionMax.SetValidValue((float) XTrend.High2);
