...
namespace GS.Trade.Strategies
{
    public class Z00705 : Z007
    {
        ...
        protected override int Entry()
        {
            ...
            {
                if (StartLongSignal1 != 0 || StartShortSignal1 != 0)
                {
                    if (StartLongSignal1 != 0)
                        if (LongEntry(StartLongSignal1))
                            return +1;
                    if (StartLongSignal2 != 0)
                        if (LongEntry(StartLongSignal2))
                            return +1;
                    if (StartShortSignal1 != 0)
                        if (ShortEntry(StartShortSignal1))
                            return -1;
                    if (StartShortSignal2 != 0)
                        if (ShortEntry(StartShortSignal2))
                            return -1;
                    return 0;
                }
                var longOrShort = LongOrShort();
        ...
        private void ChangeMode123()
        {
            ...
            if (Position.IsOpened)
            {
                ...
                {
                    var ctr = SafeContracts3 == 0 ? SafeContracts2 : SafeContracts3; 
                    Mode = Position.Quantity <= ctr ? 8 : 4;
                }
