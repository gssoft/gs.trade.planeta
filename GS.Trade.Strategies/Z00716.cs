...
namespace GS.Trade.Strategies
{
    public class Z00716 : Z007
    {
        ...
        protected PositionMinMax MyPositionStop = new PositionMinMax();
        ...
        protected override void Prefix()
        {
            if (Position.IsLong)
            {
                MyPositionStop.SetIfLessThan((float)XTrend.TrLow1);
                PositionMin.Value = MyPositionStop.Value;
            }
            else if (Position.IsShort)
            {
                MyPositionStop.SetIfGreaterThan((float)XTrend.TrHigh1);
                PositionMax.Value = MyPositionStop.Value;
            }
        ...
        private bool ShortEntry(int signal)
        {
            ...
            if (
                ...
                (signal == 0 && Mode == 6 && Position.IsShort && (TakeShort(EntrySignal61) || TakeShort(EntrySignal62))) ||
                (signal == 0 && Mode == 6 && Position.IsLong && (TakeShort(EntrySignal63) || TakeShort(EntrySignal64))) ||
        ...
        private bool LongEntry(int signal)
        {
            ...
            if (
                ...
                (signal == 0 && Mode == 6 && Position.IsLong && (TakeLong(EntrySignal61) || TakeLong(EntrySignal62))) ||
                (signal == 0 && Mode == 6 && Position.IsShort && (TakeLong(EntrySignal63) || TakeLong(EntrySignal64))) ||
            ...
            if (Position.IsOpened )
            {
                if (Position.Quantity < Contracts)
                {
                    if (Mode == 1 && _previousMode == 0)
                        Mode = 2;
                    // else if (Mode == 2 || Mode == 3 || Mode == 4 || Mode == 8)
                    else
                    {
                        //if ( // TrendMode == 1 &&
                        ...
                        if (
                            (
                                //TrendMode * XTrend.Trend55.Value > 0 && 
                                // XTrend.Ma > XTrend.TrLow1 &&  Position.IsLong) ||
                                Position.IsLong 
                                && MyPositionStop.HasValue && MyPositionStop.IsLessThan((float) XTrend.Ma)
                                ) ||
                            (
                                //TrendMode * XTrend.Trend55.Value < 0 && 
                                //XTrend.Ma < XTrend.TrHigh1 && Position.IsShort)
                                Position.IsShort 
                                    && MyPositionStop.HasValue && MyPositionStop.IsGreaterThan((float) XTrend.Ma)
                                )
                            )
                        {
                            if ((Position.IsLong && XTrend.Ma > (double) Position.Price1) ||
                                (Position.IsShort && XTrend.Ma < (double) Position.Price1))
                                Mode = 2;
                            else
                                Mode = 8;
