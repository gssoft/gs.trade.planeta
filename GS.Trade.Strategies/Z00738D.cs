    ...
    public class Z00738D : Z007
    {
        ...
        private bool ShortEntry(int signal)
        {
            ...
            if (!TrEntryEnabled.Value)
                return false;
            if (
                (signal == 0 && Mode == 1 && IsShortEntryUpToTrend && (TakeShort(EntrySignal11) || TakeShort(EntrySignal12))) ||
        ...
        private bool LongEntry(int signal)
        {
            ...
            if (!TrEntryEnabled.Value)
                return false;
            if (
                (signal == 0 && Mode == 1 && IsLongEntryUpToTrend && (TakeLong(EntrySignal11) || TakeLong(EntrySignal12))) ||
                ...
                if (Position.Quantity < Contracts)
                {
                    ...
                    if (IsPosUpToTrend)
                    {
                        if (IsPosPositive)
                        {
                            ...
                            Mode = IsTotalDailyPnlPositive ? 2 : 2;
                        ...
                        else
                        {
                            ...
                            // Mode = 2;
                            // Mode = 3;
                            Mode = IsTotalDailyPnlPositive ? 2 : 4;
                        }
                    }
                    else
                    // Second Phase After Main Goal is Scored
                    {
                        ...
                        else
                        {
                            ...
                            // Mode = 4; // Real
                            // Mode = 6;
                            // Mode = 6; // best now 10.10.17

                            // 30.09.2017
                            // Mode = IsTotalDailyPnlPositive ? 3 : 4;
                            //if (IsTotalDailyPnlPositive)
                            //    Mode = 4;
                            //else
                            //    Mode = 3;
                ...
                else if (Position.Quantity >= Contracts)
                {
                    if (IsPosUpToTrend)
                    {
                        if (IsPosPositive)
                        {
                            ...
                            Mode = IsTotalDailyPnlPositive ? 6 : 4;
                        }
                        else
                        //   Mode = 8; // 32,000 ---
                    ...
                    else
                    {
                        ...
                        if (IsPosPositive)
                        {
                            Mode = IsTotalDailyPnlPositive ? 4 : 6;
                            Mode = 6;
                        }
                        else
                        {
