...
namespace GS.Trade.Strategies
{
    public class Z0073523A1 : Z007
    {
        ...
        // 2018.05.31
        // Classic variant: Static Init SL (ISL), Smart Running SL (RSL) : IsLong && IsUp && swCnt > 0
        // Static Init SL (ISL) swCnt = 0, Running SL (RSL)
        // RSL = if(IsLong && RSL < Low2) then SL = RSL esle SL = Low2 - VolatilyUnit 2;
            ...
            if (Position.IsShort)
            {
                if (XTrend.IsDown)
                {
                    if (PositionMin.HasValue)
                    {
                        ...
                        // var 73523A2: SL = RSLprev : stay at previous place
                        else
