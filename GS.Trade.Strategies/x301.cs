...
namespace GS.Trade.Strategies
{
    ...
    //    private BarSeries _bars;
        ...
        public X301(ITradeContext tx, string name, string code, Ticker ticker, uint timeInt)
        ...
        public override void Main()
        {
            ...
            if (    _xma018.Trend < 0 && _xma018.IsImpulse && 
                    Ticker.Ask.CompareTo(((Xma018.Item)(_xma018.LastItemCompleted)).Ma) < 0 &&
                        (
                        ( Position.Pos < 0 && _swingCount >= _swingCountShort) || ( Position.Pos == 0 && _xMainTrend.Trend > 0 )
                        )
               )
            {
                ...
                TradeTerminal.SetLimitOrder(TradeAccount.Code, Code, Ticker.ClassCode, Ticker.Code,
                                            OrderOperationEnum.Buy, _currentLimitPrice, contract,
                                                DateTime.Today.Add(new TimeSpan(23, 49, 15)), _comment);
            ...
            // ******************************** Short Entry *****************
            else if (_xma018.Trend > 0 && _xma018.IsImpulse &&
                    Ticker.Bid.CompareTo(((Xma018.Item)(_xma018.LastItemCompleted)).Ma) > 0 &&
                        (
                        (Position.Pos > 0 && _swingCount >= _swingCountLong) || (Position.Pos == 0 && _xMainTrend.Trend < 0)
                        )
               )
            {              
                ...
                TradeTerminal.SetLimitOrder(TradeAccount.Code, Code, Ticker.ClassCode, Ticker.Code,
                                            OrderOperationEnum.Sell,
                                            _currentLimitPrice, contract, DateTime.Today.Add(new TimeSpan(23, 49, 15)),
                                            _comment);
