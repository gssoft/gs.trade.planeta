...
namespace GS.Trade.Strategies
{
    public class Z00513 : Z005
    {
        ...
        {
            if (Position.IsOpened && Position.Quantity > SafeContracts2)
            {
                if (Position.IsShort)
                {
                    if (LongEntry(-1))
            ...
            else if ( (Position.IsOpened && Position.Quantity <= SafeContracts2) || Position.IsNeutral)
            {
                var longOrShort = Position.IsOpened ? LongOrShortWithOpen() : LongOrShortNoOpen();
        ...
        private bool IsSafeEntry(int pos)
        {
            if (pos > 0 && XTrend.IsUp)
            {
                double high2;
        ...
        private int LongOrShortWithOpen()
        {
            if (!PositionMax.IsValid || !PositionMin.IsValid)
                return 0;
            if (((float)(XTrend.Ma)).IsLessThan(PositionMedian))
                return +1;
            return -1;
        }
        private int LongOrShortNoOpen()
        {
            if (XTrend.IsUp)
            {
                double high2;
        ...
        private bool ShortEntry(int dir)
        {
            int entrySignal = 0;
            int entrySignal2 = 0;
            int entryMode = 0;
            ...
            if ((TakeShort(entrySignal) ||
                (dir < 0 && PositionStop.IsGreaterThan((float)XTrend.Ma) && TakeShort(entrySignal2)) ||
                (dir > 0 && TakeShort(entrySignal2))
                ) && _swingCountEntry >= swCntEntry  // &&
                //  (
               ...
               )
            {
                if (XTrend.GetPrice5(-1, EntryPriceId, out _currentLimitPrice, out _comment))
                {
                    ...
                    TradeContext.Evlm(EvlResult.INFO, EvlSubject.TECHNOLOGY, StrategyTickerString,
                        "Entry=" + sig, "ShortEntry @ " + _currentLimitPrice, "ShortEntry", "");
                    return true;
                }
                if (CancelOrderMode > 0)
                {
        ...
        private bool LongEntry(int dir)
        {
            int entrySignal = 0;
            int entrySignal2 = 0;
            int entryMode = 0;
            ...
            if ((TakeLong(entrySignal) ||
                (dir < 0 && PositionMax.IsLessThan((float)XTrend.Ma) && TakeLong(entrySignal2)) ||
                (dir > 0 && TakeLong(entrySignal2))
                )
                && _swingCountEntry >= swCntEntry

                //  (
                //  (Position.IsLong &&
                //   (XTrend.Ma.CompareTo((double)Position.LastTradeBuyPrice) < 0 || LastPositionChanged == PositionChangedEnum.ReSizedDown)) ||
               ...
               )
            {
                ...
                if (XTrend.GetPrice5(+1, EntryPriceId, out _currentLimitPrice, out _comment))
                {
                    ...
                    TradeContext.Evlm(EvlResult.INFO, EvlSubject.TECHNOLOGY, StrategyTickerString,
                        "Entry=" + sig, "LongEntry @ " + _currentLimitPrice, "LongEntry", "");
        ...
        protected override int ShortExit2()
        {
            ...
            if (PositionStop.IsLessThan((float)XTrend.Ma) && TakeLong(ExitSignal1))
            {
                if (XTrend.GetPrice5(+1, ExitPriceId, out _currentLimitPrice, out _comment))
                {
                    _tryEnterMode = -1;
        ...
        protected override int LongExit2()
        {
            ...
            if (PositionStop.IsGreaterThan((float)XTrend.Ma) && TakeShort(ExitSignal1))
            {
                if (XTrend.GetPrice5(-1, ExitPriceId, out _currentLimitPrice, out _comment))
                {
                    _tryEnterMode = -1;
        ...
        private bool ChangeMode()
        {
            if (Position.IsLong)
            {
                ...
                if (_mode == 1 && _richTarget3 &&
                        (
                        TakeShort(ChangeModeCondSignal3) || (PositionMin.IsGreaterThan((float)XTrend.Ma))
                        )
                   )
                {
                    _mode = 3;
            ...
            else if (Position.IsShort)
            {
                ...
                if (_mode == 1 && _richTarget3 &&
                    (
                        TakeLong(ChangeModeCondSignal3) || (PositionMax.IsLessThan((float)XTrend.Ma))
                    )
                    )
                {
                    _mode = 3;
