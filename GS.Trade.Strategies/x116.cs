...
{
    public partial class X116 : Strategy
    {
        [XmlIgnore]
        ...
        public X116(ITradeContext tx, string name, string code, Ticker ticker, uint timeInt)
        ...
        //    _xma018.Init();
          ...
          //  _xAvrg = (XAverage)TradeContext.RegisterTimeSeries(new XAverage("XAverage", Ticker, (int)TimeInt, Ma1Length));
          //  _xAvrg.Init();
            ...
            _xMainTrend = _xma018;
          //  XMainTrend = (Xma018)TradeContext.RegisterTimeSeries(new Xma018("Xma18", Ticker, (int)TimeInt2, Ma2Length, Ma2AtrLength, Ma2KAtr, Ma2Mode));
          //  XMainTrend.Init();
        ...
        public override void Main()
        {
          //  if (TimePlan.IsTimeToRest) return;
            ...
            if (Position.IsShort)
            {
                ...
                if (_xTrend.TakeLong(SignalIndex) && _swingCount >= _swingCountShort /*&& _xTrend.Ma.CompareTo(_xAvrg.Ma) > 0*/)
                {
                    _currentLimitPrice = _xma018.GetPrice(+1, EntryMode, out _comment);
                    _comment = "Reverse to Long:[" + SignalIndex + "] "  + _comment;
            ...
            {
                // ************************** Reverse *******************************
                if (_xTrend.TakeShort(SignalIndex) && _swingCount >= _swingCountLong /*&& _xTrend.Ma.CompareTo(_xAvrg.Ma) < 0*/)
                {
                    _currentLimitPrice = _xma018.GetPrice(-1, EntryMode, out _comment);
                    _comment = "Reverse To Short:[" + SignalIndex + "] " + _comment;
            ...
            else // Position is Neutral 10000000100000101
            {
                ...
                */
                ...
                #region 0x20 Entry ContrTrend After H2 L2 BreakOut with StopZone2
                    ...
                    if (_xTrend.TakeShort(SignalIndex)  /*&& _xTrend.Ma.CompareTo(_xAvrg.Ma) < 0*/)
                    {
                        _currentLimitPrice = _xma018.GetPrice(-1, EntryMode, out _comment);
                        _comment = "Short Entry:[" +SignalIndex+ "] " + _comment;
                    ...
                    //if (_xTrend.IsDown2 && _xTrend.IsUp && _swingCount >= _swingCountLong
                    //    && _xTrend.High.CompareTo(_xTrend.Low2 + Ma1KAtr * KAtrStopZone2 * _xAtr.LastAtrCompleted) > 0)
                    if (_xTrend.TakeLong(SignalIndex)  /*&& _xTrend.Ma.CompareTo(_xAvrg.Ma) > 0*/)
                    {
                        _currentLimitPrice = _xma018.GetPrice(+1, EntryMode, out _comment);
                        _comment = "Long Entry:[" + SignalIndex + "] " + _comment;
