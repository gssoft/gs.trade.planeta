...
namespace GS.Trade.Strategies
{
    public class Z00514 : Z005
    {
        ...
        private new bool ShortEntry()
        {
            ...
            if ((TakeShort(entrySignal) || ( /*IsSafeContracts && */ TakeShort(entrySignal2))) && _swingCountEntry >= swCntEntry  && TrEntryEnabled.Value // &&
                //  (
               ...
               )
            {
                ...
                if (XTrend.GetPrice5(-1, EntryPriceId, out _currentLimitPrice, out _comment))
                {
                    ...
                    if (TakeShort(entrySignal2) && entrySignal2 == 253 && XTrend.IsUp)
                        TradeContext.Evlm(EvlResult.INFO, EvlSubject.TECHNOLOGY, StrategyTickerString,
                            "Short Up Entry=" + entrySignal2, "ShortEntry @ " + _currentLimitPrice, "TradeEnabled=" + TrEntryEnabled.Value, "");


                    var sig = TakeShort(entrySignal) ? entrySignal : entrySignal2;
                    TradeContext.Evlm(EvlResult.INFO, EvlSubject.TECHNOLOGY, StrategyTickerString,
                        "Entry=" + sig, "ShortEntry @ " + _currentLimitPrice, "TradeEnabled="+TrEntryEnabled.Value, "");
                    return true;
                }
                if (CancelOrderMode > 0)
                {
        ...
        private new bool LongEntry()
        {
            ...
            if ((TakeLong(entrySignal) || (/*IsSafeContracts &&*/ TakeLong(entrySignal2))) && _swingCountEntry >= swCntEntry && TrEntryEnabled.Value// &&
                //  (
                //  (Position.IsLong &&
               ...
               )
            {
                ...
                if (XTrend.GetPrice5(+1, EntryPriceId, out _currentLimitPrice, out _comment))
                {
                    ...
                    if (TakeLong(entrySignal2) && entrySignal2 == 253 && XTrend.IsDown)
                    TradeContext.Evlm(EvlResult.INFO, EvlSubject.TECHNOLOGY, StrategyTickerString,
                        "Long Down Entry=" + sig, "LongEntry @ " + _currentLimitPrice, "LongEntry", "");
