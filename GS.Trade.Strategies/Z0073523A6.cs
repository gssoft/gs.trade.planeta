...
namespace GS.Trade.Strategies
{
    public class Z0073523A6 : Z007
    {
        ...
        protected override int Entry()
        {
            ...
            if (Position.IsNeutral && LastSignalActivated == 19) // && _swingCountEntry < 1)
            {
                if (IsNeutral && Math.Abs(LastTradeMaValue - XTrend.Ma).IsLessThan(VolatilityUnit2 * 1d))
                    //    return 0;
                    return 0;
            }
        ...
        private bool _isWarningPriceLevelFromPosMaxReached;
        private void ChangeMode123()
        {
            var previousMode = Mode;
            if (Position.IsOpened )
            {
                if (!_isWarningPriceLevelFromPosMaxReached)
                    _isWarningPriceLevelFromPosMaxReached = IsWarningPriceLevelFromPosMaxReached;
