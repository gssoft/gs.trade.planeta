...
namespace GS.Trade.Strategies
{
    ...
    public class X102 : Strategy
    {
      ...
      //  private BarSeries _bars;
        ...
        public X102(ITradeContext tx, string name, string code, Ticker ticker, uint timeInt)
        ...
        public override void Main()
        {
          //  if (TimePlan.IsTimeToRest) return;
            ...
            else if (Position.IsLong)
            {
                ...
                // *****************************  Long Exit ********************************
                #region Signal 01 Long Trend Exit Ma TrendChanged
                if ( (SignalMap & 0x1 ) != 0  &&
                ...
                #endregion
                #region Signal 02 Long Trend Exit - Lustra
                if ( (SignalMap & 0x2) != 0 &&
                    _xTrend.IsUp2 && _xTrend.IsDown && _swingCount >= _swingCountLong
...
#region Postion is Neutral
            else // Position == 0
            {
             ...
             //   if ( (SignalMap & 0x8000) != 0 )
             //   {
                    if (_xTrend.IsUp && ! _xTrend.IsDown2)
                    {
                        _currentLimitPrice = _xma018.GetPrice(+1, EntryMode, out _comment);
                    ...
                    else if (_xTrend.IsDown && ! _xTrend.IsUp2)
                    {
                        _currentLimitPrice = _xma018.GetPrice(-1, EntryMode, out _comment);
             ...
             //   }
                #endregion
                #region Entry After Trend
                if ((SignalMap & 0x4000) != 0)
                {
                ...
                // Long Entry After UpTrend Exit
                    if ( (SignalMap & 0x8) != 0 &&
                        _xTrend.IsUp2 && _xTrend.IsDown
                        && _xTrend.Low.CompareTo(_xTrend.Low2 + KAtrStopZone1 * Ma1KAtr * _xAtr.LastAtrCompleted) < 0)  //  First was <=
                    {
                    ...
                    if ((SignalMap & 0x8) != 0 &&
                        _xTrend.IsDown2 && _xTrend.IsUp
                        && _xTrend.High.CompareTo(_xTrend.High2 - KAtrStopZone1 * Ma1KAtr * _xAtr.LastAtrCompleted) > 0) // At First was >=
                    {
                ...
                #region Signal 04
                    ...
                    {
                        _currentLimitPrice = _xma018.GetPrice(+1, 0, out _comment);
                        _comment = "Long Entry After UpTrend Exit. L=" + _xTrend.Low + " <= H3=" + _xTrend.High3 + " " + _comment;
                        ...
                        )
                    {
                        _currentLimitPrice = _xma018.GetPrice(-1, 0, out _comment);
                        //_comment = "Short Entry After DownTrend Exit. " + _comment;
                        _comment = "Short Entry After DownTrend Exit. H=" + _xTrend.High + " >= L3=" + _xTrend.Low3 + " " + _comment;
