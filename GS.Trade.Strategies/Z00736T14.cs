...
namespace GS.Trade.Strategies
{
    public class Z00736T14 : Z007
    {
        ...
        {
            // if (_swingCountEntry < SwingCountEntry) return false;

            //if (!TrEntryEnabled.Value)
            //    return false;
            // if (_swingCountEntry < SwingCountEntry && Mode != 8) return false;
           // if (!IsSwingCountEntryReached && !IsSwingCountEntry2Reached && !IsSwingCountEntry3Reached) return false;
            if (
                (IsSwingCountEntryReached && signal == 0 && Mode == 1 && IsPrimTrendDown && (TakeShort(EntrySignal11) || TakeShort(EntrySignal12))) ||
                // (IsXTrend2EntryEnable && signal == 0 && Mode == 2 && Position.IsShort && (TakeShort2(EntrySignal21) || TakeShort2(EntrySignal22))) ||
                (IsXTrendEntryEnable && signal == 0 && Mode == 2 && Position.IsShort && (TakeShort(EntrySignal21) || TakeShort(EntrySignal22))) ||
                (IsXTrendEntryEnable && signal == 0 && Mode == 2 && Position.IsLong && (TakeShort(EntrySignal23) || TakeShort(EntrySignal24))) ||
                (IsXTrendEntryEnable && signal == 0 && Mode == 3 && Position.IsShort && (TakeShort(EntrySignal31) || TakeShort(EntrySignal32))) ||
                // (IsXTrendEntryEnable && signal == 0 && Mode == 3 && Position.IsLong && (TakeShort(EntrySignal33) || TakeShort(EntrySignal34))) ||
                (IsXTrend3EntryEnable && signal == 0 && Mode == 3 && Position.IsLong && (TakeShort3(EntrySignal33) || TakeShort3(EntrySignal34))) ||
                (IsXTrendEntryEnable && signal == 0 && Mode == 5 && Position.IsShort && (TakeShort(EntrySignal51) || TakeShort(EntrySignal52))) ||
                //(IsXTrendEntryEnable && signal == 0 && Mode == 5 && Position.IsLong && (TakeShort(EntrySignal53) || TakeShort(EntrySignal54))) ||
                (IsXTrend3EntryEnable && signal == 0 && Mode == 5 && Position.IsLong && (TakeShort3(EntrySignal53) || TakeShort3(EntrySignal54))) ||
                ...
                (IsXTrendEntryEnable && signal == 0 && Mode == 8 && Position.IsLong && (TakeShort(EntrySignal83) || TakeShort(EntrySignal84))) ||
                (IsXTrendEntryEnable && signal != 0 && TakeShort(signal))
                )
            {
                if (XTrend.GetPrice6(-1, EntryPriceId, LastSignalActivated, out _currentLimitPrice, out _comment))
                {
                    ...
                    TradeContext.Evlm(EvlResult.INFO, EvlSubject.TECHNOLOGY, StrategyTickerString, "Signal", sigStr,
                        sigStr + " @ " + _currentLimitPrice, $"Mode: {Mode} TrMaxCntr: {TrMaxContracts} Sig: {sigStr}");
        ...
        private bool LongEntry(int signal)
        {
           ...
           // if (!IsSwingCountEntryReached && !IsSwingCountEntry2Reached && !IsSwingCountEntry3Reached) return false;
            if (
                (IsSwingCountEntryReached && signal == 0 && Mode == 1 && IsPrimTrendUp && (TakeLong(EntrySignal11) || TakeLong(EntrySignal12))) ||
                ...
                (IsXTrendEntryEnable && signal == 0 && Mode == 2 && Position.IsShort && (TakeLong(EntrySignal23) || TakeLong(EntrySignal24))) ||
                (IsXTrendEntryEnable && signal == 0 && Mode == 3 && Position.IsLong && (TakeLong(EntrySignal31) || TakeLong(EntrySignal32))) ||
                // (IsXTrendEntryEnable && signal == 0 && Mode == 3 && Position.IsShort && (TakeLong(EntrySignal33) || TakeLong(EntrySignal34))) ||
                (IsXTrend3EntryEnable && signal == 0 && Mode == 3 && Position.IsShort && (TakeLong3(EntrySignal33) || TakeLong3(EntrySignal34))) ||
                (IsXTrendEntryEnable && signal == 0 && Mode == 5 && Position.IsLong && (TakeLong(EntrySignal51) || TakeLong(EntrySignal52))) ||
                //(IsXTrendEntryEnable && signal == 0 && Mode == 5 && Position.IsShort && (TakeLong(EntrySignal53) || TakeLong(EntrySignal54))) ||
                (IsXTrend3EntryEnable && signal == 0 && Mode == 5 && Position.IsShort && (TakeLong3(EntrySignal53) || TakeLong3(EntrySignal54))) ||
                ...
                (IsXTrendEntryEnable && signal == 0 && Mode == 8 && Position.IsShort && (TakeLong(EntrySignal83) || TakeLong(EntrySignal84))) ||
                (IsXTrendEntryEnable && signal != 0 && TakeLong(signal))
                )
            {
                if (XTrend.GetPrice6(+1, EntryPriceId, LastSignalActivated, out _currentLimitPrice, out _comment))
                {
        ...
        protected bool IsPosUpToTrend =>
            PrimTrend != null &&
            // ((PrimTrend.IsUp && Position.IsLong) || (PrimTrend.IsDown && Position.IsShort));
            // ((PrimTrend.IsDown5 && Position.IsLong) || (PrimTrend.IsUp5 && Position.IsShort));
            ((IsPrimTrendUp && Position.IsLong) || (IsPrimTrendDown && Position.IsShort));


        //protected bool IsLongEntryUpToTrend => PrimTrend?.IsDown5 ?? true;
        //protected bool IsShortEntryUpToTrend => PrimTrend?.IsUp5 ?? true;

        private int _previousMode;
        private int _tr;
        private void ChangeMode123()
        {
            if (TrendSafeContracts == 0) TrendSafeContracts = 5;
            var previousMode = Mode;
            if (Position.IsOpened)
            {
                if (IsSwingCountEntryReached || IsSwingCountEntry2Reached || IsSwingCountEntry3Reached)
                {
                    if (Position.Quantity < Contracts)
                    {
                        if (IsPosUpToTrend)
                        {
                            Mode = Position.Quantity <= TrendSafeContracts ? 2 : 3;
                        }
                        else
                        {
                            Mode = 4;
                    ...
                    {
                        if (MaxContractsPositionHold)
                        {
                            Mode = IsPosUpToTrend ? 7 : 4;
                        }
                        else Mode = IsPosUpToTrend ? 5 : 4;
                    }
                }
                // _swCountEntry == 0
                else
                {
                    //if (Position.Quantity <= TrendSafeContracts)
                    //{
                    //    Mode = 2;
                    //    return;
                    //}
                    // KAtrStop = TakeProfit when _swingCountEntry == 0
                    if (KAtrTakeProfit.IsEquals(0)) return;
                    if (
                        (IsLong && (XTrend.Ma - LastTradeMaValue).IsGreaterThan(2 * Volatility * KAtrTakeProfit)) ||
                        (IsShort && (-XTrend.Ma + LastTradeMaValue).IsGreaterThan(2 * Volatility * KAtrTakeProfit))
                        )
                        Mode = 8;
                    else Mode = 9;
                }
