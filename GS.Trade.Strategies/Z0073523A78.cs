...
namespace GS.Trade.Strategies
{
    public class Z0073523A78 : Z007
    {
        ...
        protected override int Entry()
        {
            ...
            //if (IsPortfolioRiskEnable && _swingCountEntry < SwingCountEntry)
            //    return 0;
            ...
            else if (Position.IsNeutral)
            {
                ...
                if (!IsAmericanSessionTradingEnabled)
                    return 0;
                //if (TrendMode * XTrend.Trend55.Value > 0)
                //{
                //    if (LongEntry(0))
                ...
                if (TrendMode == -1)
                {
                    //if (Trend55Changed.Value && XTrend.Trend55.Value < 0)
                    //{
                    ...
                    //else if (Trend55Changed.Value && XTrend.Trend55.Value > 0)
                    //{
        ...
        private bool ShortEntry(int signal)
        {
                ...
                )
            {
                ...
                if (XTrend.GetPrice6(-1, EntryPriceId, LastSignalActivated, out _currentLimitPrice, out _comment))
                {
                    ...
                    var sigLine = $"Signal: {LastSignalActivated}, Mode: {Mode}, SwCntEntry: {_swingCountEntry}, DT: {XTrend.LastItemCompletedDT}";

                    TradeContext.Evlm(EvlResult.INFO, EvlSubject.TRADING, StrategyTickerString, sigLine, operStr, descrStr, "");
        ...
        private bool LongEntry(int signal)
        {
                ...
                )
            {
                ...
                if (XTrend.GetPrice6(+1, EntryPriceId, LastSignalActivated ,out _currentLimitPrice, out _comment))
                {
                    ...
                    var sigLine = $"Signal: {LastSignalActivated}, Mode: {Mode}, SwCntEntry: {_swingCountEntry}, DT: {XTrend.LastItemCompletedDT}, Acc: {TradeAccountCode} ";
            ...
            if (Position.IsOpened )
            {
                ...
                else if (Position.Quantity >= Contracts)
                {
                    ...
                    if (IsPosPositive)
                    {
                        if (_swingCountEntry <= 0)
                        {
                            Mode = IsTakeProfitAtrsValueReached || IsTakeProfitAbsValueReached 
                                ? 2
                                : 7;
                            return;
                        }
                        if (IsProfitLevelReached)
                        {
                            Mode = 2;
                            return;
                        }
        ...
        protected override int PortfolioRiskOperate(int oper)
        {
            ...
            //            (
            //            Position.IsNeutral && ShortRequest(Contract) > 0
            ...
            if (oper < 0)
            {
                if (IsOpened)
                    return -1;
                if (IsNeutral && ShortRequest((int) Contract) > 0)
                {
                    // Only EntrySignal11
                    if (EntrySignal12 == 0)
                        return -1;
                    // EntrySignal12 != 0
                    if (
                            (PortfolioRisk.ShortsCount <= 0 && LastSignalActivated == EntrySignal11)
                        ||  (PortfolioRisk.ShortsCount > 0  && LastSignalActivated == EntrySignal12)
                        )
                        return -1;
                    // return 0;
                }
            ...
            //        //(Position.IsOpened && BuySellRequest(Contract) > 0) ||
            //        (Position.IsNeutral && LongRequest(Contract) > 0
            ...
            if (oper > 0)
            {
                if (IsOpened)
                    return +1;
                if (IsNeutral && LongRequest((int) Contract) > 0)
                {
                    // Only EntrySignal11
                    if (EntrySignal12 == 0)
                        return +1;
                    // EntrySignal12 != 0
                    if (
                           (PortfolioRisk.LongsCount <= 0 && LastSignalActivated == EntrySignal11)
                        || (PortfolioRisk.LongsCount > 0 && LastSignalActivated == EntrySignal12)
                        )
                        return +1;
