...
namespace GS.Trade.Strategies
{
    public class X11809 : X118
    {
        ...
        protected override bool LongReverse()
        {
            ...
            if (_mode == 1 && _richTarget && ReverseSignal1 > 0 && XTrend.TakeShort(ReverseSignal1)
            ...
            if (_mode == 2 && _richTarget && ReverseSignal2 > 0 && XTrend.TakeShort(ReverseSignal2)
                && _swingCountReverse >= SwingCountReverse)
            {
                if (XTrend.GetPrice5(-1, ReversePriceId, out _currentLimitPrice, out _comment))
                {
        ...
        {
            if (_mode == 1 && _richTarget && ExitSignal1 > 0 && XTrend.TakeLong(ExitSignal1) && _swingCountExit >= SwingCountExit)
            {
                if (XTrend.GetPrice5(+1, ExitPriceId, out _currentLimitPrice, out _comment))
                {
            ...
            if (_mode == 2 && _richTarget && ExitSignal2 > 0 && XTrend.TakeLong(ExitSignal2) && _swingCountExit >= SwingCountExit)
            {
                if (XTrend.GetPrice5(+1, ExitPriceId, out _currentLimitPrice, out _comment))
                {
        ...
        {
            if (_mode == 1 && _richTarget && ExitSignal1 > 0 && XTrend.TakeShort(ExitSignal1) && _swingCountExit >= SwingCountExit)
            {
                if (XTrend.GetPrice5(-1, ExitPriceId, out _currentLimitPrice, out _comment))
                {
            ...
            if (_mode == 2 && _richTarget && ExitSignal2 > 0 && XTrend.TakeShort(ExitSignal2) && _swingCountExit >= SwingCountExit)
            {
                if (XTrend.GetPrice5(-1, ExitPriceId, out _currentLimitPrice, out _comment))
                {
