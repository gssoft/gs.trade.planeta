...
namespace GS.Trade.Strategies
{
    ...
    //    private BarSeries _bars;
        ...
        public X100(ITradeContext tx, string name, string code, Ticker ticker, uint timeInt)
        ...
        public override void Main()
        {
            ...
            {
                if ( (Position.Pos < 0 && _swingCount >= _swingCountShort) ||
            ...
            {
                if ( (Position.Pos > 0 && _swingCount >= _swingCountLong) ||
                     (Position.Pos == 0 && _xMainTrend.Trend < 0) )                  
                        _currentLimitPrice = _xma018.GetPrice(-1, EntryMode, out _comment);
                else return;
