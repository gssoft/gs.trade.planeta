...
namespace GS.Trade.Strategies.Portfolio
{
    ...
    public class Portfolio : Element2<string, IStrategy, Containers5.ListContainer<string, IStrategy>>, IPortfolioRisk
    {
        [XmlIgnore]
        ...
        public int SideSizeHighRisk { get; set; }
        public int SideSizeNormRisk { get; set; }
        public float MaxSideInitValue { get; set; }
        //public long MaxOneSidePositionSize => MaxOneSideSize;
        ...
        public bool IsSellRequestEnabled()
        {
            ...
            //        (PortfolioPosition < 0 && PortfolioPositionSize + SellRequestCount < MaxSideSize);
            return SellsAvailable > 0;
        }
        public bool IsBuyRequestEnabled()
        {
            //return  PortfolioPosition <= 0 ||
            //        (PortfolioPosition > 0 && PortfolioPositionSize + BuyRequestCount < MaxSideSize);
            return BuysAvailable > 0;
        }
        public long BuysAvailable => PortfolioPosition >= 0
            ? MaxSideSize - PortfolioPositionSize - BuyRequestCount
            : MaxSideSize;
        public long SellsAvailable => PortfolioPosition <= 0
            ? MaxSideSize - PortfolioPositionSize - SellRequestCount
            : MaxSideSize;

        public int LongRequestsCount => Collection.Items.Count(s => s.LongContractsRequest > 0);
        public long ShortRequestsCount => Collection.Items.Count(s => s.ShortContractsRequest > 0);
        public int BuyRequestCount => Collection.Items.Count(s => s.BuyContractsRequest > 0);
        public int SellRequestCount => Collection.Items.Count(s => s.SellContractsRequest > 0);
        ...
        public long ShortsAvailable =>
            MaxSideCount - ShortsCount - ShortRequestsCount > 0
                ...
                : 0;
        public bool IsSideSizeHighRisk => PortfolioPositionSize >= SideSizeHighRisk;
        public bool IsSideSizeNormRisk => PortfolioPositionSize <= SideSizeNormRisk;

        public long PortfolioPositionSize => Position?.Quantity ?? 0;
        public long PortfolioPosition => Position?.Pos ?? 0;
        public Portfolio()
        {
        ...
        private void ReCalc()
        {
            ...
            long k = 0;
            long p = 0;
            ulong maxNumber = Position.LastTradeNumber;
            DateTime maxDT = Position.LastTradeDT;
            ...
            if (IsSideSizeHighRisk)
            {
                foreach (var s in Collection.Items.Where(s => s.IsHedger))
                {
            ...
            Position.Comment = $"Longs:{LongsCount} Available:{LongsAvailable} " +
                               ...
                               $"BuyAvl:{BuysAvailable} SellAvl:{SellsAvailable} " +
                               $"BuyRqst:{BuyRequestCount} SelRqst:{SellRequestCount} " +
                               $"MaxSideCnt:{MaxSideCount}; MaxSideSize:{MaxSideSize} SideSizeRH:{SideSizeHighRisk} SideSizeRN:{SideSizeNormRisk} " +
                               $"Count:{Count} PosQty:{PortfolioPositionSize} Pos:{PortfolioPosition} " +
                               $"IsRiskHigh:{IsSideSizeHighRisk} IsRiskNorm:{IsSideSizeNormRisk}";
        ...
        public string SellRequestShortInfo => $"SellsAvailable:{SellsAvailable} MaxSideSize:{MaxSideSize} PortfPos:{PortfolioPosition} SellRqst:{SellRequestCount}";
        public void SetMaxSideSize(int maxsidesize)
        {
            ...
            Evlm2(EvlResult.INFO, EvlSubject.TECHNOLOGY, Code, "MaxSideSize",
                methodname, $"MaxSideSize:{MaxSideSize}",ToString());
        }
        ...
        public override void Init()
        {
            ...
            MaxSideSize = (int) _maxContracts;
            Evlm2(EvlResult.SUCCESS, EvlSubject.TECHNOLOGY, "Portfolio", Code, "Init()",
                $"MaxContractsToOpen:{_maxContracts} DailyProfitLimit:{Position.DailyProfitLimit} MaxSideCnt:{MaxSideCount} MaxSideSize:{MaxSideSize} MaxSideSizeHR:{SideSizeHighRisk}",
                ToString());
        }
