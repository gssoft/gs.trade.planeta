    ...
    public class Z00518 : Z005
    {
        ...
        protected override void PositionIsChangedEventHandler2(IPosition2 op, IPosition2 np, PositionChangedEnum changedResult)
        {
            ...
            if (np.IsOpened)
            {
                ...
                {
                    if (changedResult == PositionChangedEnum.ReSizedUp && np.Quantity >= 3)
                        Mode = 1;
                    else if (changedResult == PositionChangedEnum.ReSizedDown && np.Quantity < 2)
                        Mode = 2;
                }
                else
                    Mode = 1;
                if (np.IsOpened && op.IsNeutral)
                {
                    if (np.IsLong)
                    {
                        Mode = XTrend.Ma.IsLessThan((double) np.Price1) ? 1 : 2;
                    }
                    if (np.IsShort)
                    {
                        Mode = XTrend.Ma.IsGreaterThan((double)np.Price1) ? 1 : 2;
                    }
                }
                if (np.IsNeutral)
                    Mode = 2;
        ...
        private int LongOrShort()
        {
            ...
            {
                double high2;
                if (XTrend.HaveHigher((int)TimeInt * 60 * KMinFarFromHere, XTrend.Ma, out high2))
                {
                    var dlow = XTrend.Ma - XTrend.Low2;
                    var dhigh = high2 - XTrend.Ma;
                    if (dhigh.CompareTo(dlow) > 0)
                        return +1;
                    return -1;
                }
                return -1;
            ...
            {
                double low2;
                if (XTrend.HaveLower((int)TimeInt * 60 * KMinFarFromHere, XTrend.Ma, out low2))
                {
                    var dhigh = XTrend.High2 - XTrend.Ma;
                    var dlow = XTrend.Ma - low2;
                    if (dlow.CompareTo(dhigh) > 0)
                        return -1;
                    return +1;
                }
                return +1;
        ...
        private new bool ShortEntry()
        {
            ...
            if (
                (Mode == 2 && (TakeShort(entrySignal) || TakeShort(entrySignal2)) && _swingCountEntry >= swCntEntry)
                ||
                (Mode == 1 && _swingCountEntry >= swCntEntry && Position.IsShort &&
                (
                (TrMaxContracts.Value && TakeShort(entrySignal) && PositionMax.IsGreaterThan((float)XTrend.Ma)) ||
                (!TrMaxContracts.Value && TakeShort(entrySignal))
                )
                )
                 ||
                (Mode == 1 && _swingCountEntry >= swCntEntry && (Position.IsLong || Position.IsNeutral) && TakeShort(entrySignal))
               )
            {
                if (XTrend.GetPrice5(-1, EntryPriceId, out _currentLimitPrice, out _comment))
                {
                    ...
                    var sig = TakeShort(entrySignal) ? entrySignal : entrySignal2;
                    TradeContext.Evlm(EvlResult.INFO, EvlSubject.TECHNOLOGY, StrategyTickerString, "Signal", "ShortEntry",
                        "ShortEntry(" + sig + ") @ " + _currentLimitPrice, String.Format("Mode: {0} TrMaxCntr: {1} Sig: {2}",
        ...
        private new bool LongEntry()
        {
            ...
            if ((Mode == 2 && (TakeLong(entrySignal) || TakeLong(entrySignal2)) && _swingCountEntry >= swCntEntry)
                ||
                (Mode == 1 && _swingCountEntry >= swCntEntry && Position.IsLong &&
                (
                (TrMaxContracts.Value && TakeLong(entrySignal) && PositionMin.IsLessThan((float)XTrend.Ma)) ||
                (!TrMaxContracts.Value && TakeLong(entrySignal))
                )
                )
                ||
                (Mode == 1 && _swingCountEntry >= swCntEntry && (Position.IsShort || Position.IsNeutral) && TakeLong(entrySignal))
               )
            {
                ...
                if (XTrend.GetPrice5(+1, EntryPriceId, out _currentLimitPrice, out _comment))
                {
                    ...
                    var sig = TakeLong(entrySignal) ? entrySignal : entrySignal2;
                    TradeContext.Evlm(EvlResult.INFO, EvlSubject.TECHNOLOGY, StrategyTickerString, "Signal", "LongEntry",
                        "LongEntry(" + sig + ") @ " + _currentLimitPrice, String.Format("Mode: {0} TrMaxCntr: {1} Sig: {2}",
                                                                                    Mode, TrMaxContracts, sig));
