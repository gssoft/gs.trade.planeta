    ...
    public class Z00738B : Z007
    {
        ...
        private bool ShortEntry(int signal)
        {
            ...
            if (!TrEntryEnabled.Value)
                return false;
            if (
                (signal == 0 && Mode == 1 && IsShortEntryConditionEnabled && (TakeShort(EntrySignal11) || TakeShort(EntrySignal12))) ||
        ...
        private bool LongEntry(int signal)
        {
            ...
            if (!TrEntryEnabled.Value)
                return false;
            if (
                (signal == 0 && Mode == 1 && IsLongEntryConditionEnabled && (TakeLong(EntrySignal11) || TakeLong(EntrySignal12))) ||
        ...
        protected bool IsPosUpToTrend =>
            ...
            ((IsLongEntryUpToTrend) || (IsShortEntryUpToTrend));

        //protected bool IsLongEntryUpToTrend => PrimTrend?.IsUp5 ?? true;
        //protected bool IsShortEntryUpToTrend => PrimTrend?.IsDown5 ?? true;

        public override bool IsLongEntryConditionEnabled => 
            TrendMode > 0
                ? PrimTrend?.IsUp ?? false
                : PrimTrend?.IsDown ?? false;
        public override bool IsShortEntryConditionEnabled =>
            TrendMode > 0
                ? PrimTrend?.IsDown ?? false
                : PrimTrend?.IsUp ?? false;

        //public override bool IsLongEntryConditionEnabled => (PrimTrend?.IsDown ?? false);
        //public override bool IsShortEntryConditionEnabled => PrimTrend?.IsUp ?? false;

        //public override bool IsLongEntryConditionEnabled => PrimTrend.TakeLong(212) || ! PrimTrend.TakeShort(212);
        //public override bool IsShortEntryConditionEnabled => PrimTrend.TakeShort(212) || ! PrimTrend.TakeLong(212);

        protected bool IsLongEntryUpToTrend => IsPosLong && IsLongEntryConditionEnabled;
        protected bool IsShortEntryUpToTrend => IsPosShort && IsShortEntryConditionEnabled;

        protected bool IsLongHighBreakUp => IsPosLong && (XTrend?.IsTrend55HighBreakUp ?? false);
        protected bool IsShortLowBreakDown => IsPosShort && (XTrend?.IsTrend55LowBreakDown ?? false);

        protected bool IsLongLowBreakDown => IsPosLong && (XTrend?.IsTrend55LowBreakDown ?? false);
        protected bool IsShortHighBreakUp => IsPosShort && (XTrend?.IsTrend55HighBreakUp ?? false);
        ...
        private void ChangeMode123()
        {
            ...
            if (Position.IsOpened )
            {
                if (Position.Quantity < Contracts)
                {
                    ...
                    //{
                    // 21.07.17
                    // Mode = IsPosUpToTrend ? 2 : 4;
                    // Mode = IsPosUpToTrend ? 2 : (IsPosPositive ? 8 : 4);
                    // 21.07.17
                    ...
                    if (IsPosUpToTrend)
                    {
                        if (IsPosPositive)
                        {
                            // Mode = IsLongLowReached || IsShortHighReached ? 2 : 6;
                            Mode = 2;
                        }
                        else
                        {
                            ...
                            //  Mode = 3;          // RIU7 20 -22
                            //  
                            // Mode = IsLongLowReached || IsShortHighReached ? 3 : 6;
                            // Mode = IsLongTargetReached || IsShortTargetReached ? 4 : 6;
                            // *** 2
                            //if (IsLongHighBreakUp || IsShortLowBreakDown)
                            //    Mode = 4;
                            //else
                            //    Mode = 2;
                            // *** 1
                    ...
                    else  
                    {
                        ...
                        { 
                            // *** 1,2,3
                            // Mode = IsLongHighBreakUp || IsShortLowBreakDown ? 4 : 6;
                            // *** 1
                             Mode = 4;
                        }
                    }
