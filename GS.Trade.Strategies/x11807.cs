    ...
    public class X11807 : X118
    {
        ...
        protected override void Prefix()
        {
            if (Position.IsLong && XTrend.IsUp)
            {
                ...
                if ((XTrend.Ma - (double)Position.Price1).CompareTo(KAtrTake * VolatilityUnit2) > 0)
                    _needTake = true;
            }
        ...
        {
            if(
                (ExitSignal1 > 0 && XTrend.TakeLong(ExitSignal1) && +_swingCountExit >= SwingCountExit && Mode1 == 1)
                ||
                (ExitSignal1 > 0 && XTrend.TakeLong(ExitSignal1) && +_swingCountExit >= SwingCountExit && Mode1 == 2
                && _needTake)
                )
            {
        ...
        {
            if (
                (ExitSignal1 > 0 && XTrend.TakeShort(ExitSignal1) && +_swingCountExit >= SwingCountExit && Mode1 == 1)
                ||
                (ExitSignal1 > 0 && XTrend.TakeShort(ExitSignal1) && +_swingCountExit >= SwingCountExit && Mode1 == 2
                && _needTake)
                )
            {
                _currentLimitPrice = XTrend.GetPrice3(-1, ExitPriceId, out _comment);
