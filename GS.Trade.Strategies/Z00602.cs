...
namespace GS.Trade.Strategies
{
    public class Z00602 : Z006
    {
        ...
        private new bool ShortEntry()
        {
            ...
            var bSig = TrEntryEnabled.Value &&
                       (
                       (TakeShort(entrySignal) && HaveHigher()) ||
                       (TakeShort(entrySignal) && EntrySignal13 == 0) ||
                       (TakeShort(EntrySignal13) && !HaveHigher())
                      );
            //if (bSig && TakeShort(2) && !HaveHigher())
            //    TradeContext.Evlm(EvlResult.INFO, EvlSubject.TECHNOLOGY, StrategyTickerString, "Signal", "ShortEntry(2)",
            //            "ShortEntry(2) @ " + _currentLimitPrice, String.Format("Mode: {0} TrMaxCntr: {1}",
            //                                                                        Mode, TrMaxContracts));
            if (
                (Mode == 2 && (bSig || TakeShort(entrySignal2)) && TrEntryEnabled.Value)
                ||
                (Mode == 1 && Position.IsShort && bSig
                //(
                //(TrMaxContracts.Value && bSig && PositionMax.IsGreaterThan((float)XTrend.Ma)) ||
                //(!TrMaxContracts.Value && bSig)
                //)
                )
                 ||
                (Mode == 1 && (Position.IsLong || Position.IsNeutral) && bSig)
               )
            {
                if (XTrend.GetPrice5(-1, EntryPriceId, out _currentLimitPrice, out _comment))
                {
        ...
        private new bool LongEntry()
        {
            ...
            var bSig = TrEntryEnabled.Value &&
                       (
                       (TakeLong(entrySignal) && HaveLower()) ||
                       (TakeLong(entrySignal) && EntrySignal13 == 0) ||
                       (TakeLong(EntrySignal13) && !HaveLower())
                       );
            //if (bSig && TakeLong(2) && !HaveLower())
            //    TradeContext.Evlm(EvlResult.INFO, EvlSubject.TECHNOLOGY, StrategyTickerString, "Signal", "LongEntry(2)",
            //            "LongEntry(2) @ " + _currentLimitPrice, String.Format("Mode: {0} TrMaxCntr: {1}",
            //                                                                        Mode, TrMaxContracts));

            if ((Mode == 2 && (bSig || TakeLong(entrySignal2)) && TrEntryEnabled.Value)
                ||
                (Mode == 1 && Position.IsLong && bSig
                //(
                //(TrMaxContracts.Value && bSig && PositionMin.IsLessThan((float)XTrend.Ma)) ||
                //(!TrMaxContracts.Value && bSig)
                //)
                )
                ||
                (Mode == 1 && bSig && (Position.IsShort || Position.IsNeutral))
               )
            {
                ...
                if (XTrend.GetPrice5(+1, EntryPriceId, out _currentLimitPrice, out _comment))
                {
                    ...
                    var sig = TakeLong(entrySignal) ? entrySignal : (TakeLong(EntrySignal13) ? EntrySignal13 : entrySignal2);
                    var sigStr = "LongEntry(" + sig + ")";
                    TradeContext.Evlm(EvlResult.INFO, EvlSubject.TECHNOLOGY, StrategyTickerString, "Signal", sigStr,
                        sigStr + " @ " + _currentLimitPrice, String.Format("Mode: {0} TrMaxCntr: {1} Sig: {2}",
                                                                                    Mode, TrMaxContracts, sigStr));
        ...
        private void ChangeMode123()
        {
            if (Position.IsOpened)
            {
                if (
                    (
                        Position.IsLong &&
                        XTrend.Ma.IsGreaterThan((double) Position.Price1)
                        )
                    ||
                    (
                        Position.IsShort &&
                        XTrend.Ma.IsLessThan((double) Position.Price1)
                        )
                   )
                {
                    Mode = Position.Quantity >= Contracts ? 1 : 2;
