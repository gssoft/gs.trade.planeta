...
namespace GS.Trade.Strategies
{
    public class Z00718 : Z007
    {
        ...
        // 22.08.2017
        //protected override int EntryOld()  
        //{
        ...
        private bool ShortEntry(int signal)
        {
            ...
            {
                if (XTrend.GetPrice6(-1, EntryPriceId, entrySignal, out _currentLimitPrice, out _comment))
                {
        ...
        private void ChangeMode123()
        {
            ...
            if (Position.IsOpened )
            {
                if (Position.Quantity < Contracts)
                {
                    ...
                    if ( // TrendMode == 1 &&
                        (Position.IsLong && TrendMode * XTrend.Trend55.Value > 0) ||
                        (Position.IsShort && TrendMode * XTrend.Trend55.Value < 0)
                        )
                        Mode = 2;
                    else
                        Mode = 8;
