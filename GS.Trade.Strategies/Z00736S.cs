    ...
    public class Z00736S : Z007
    {
        ...
        private bool ShortEntry(int signal)
        {
            ...
            if (!TrEntryEnabled.Value)
                return false;
            if (Position.IsNeutral && !IsShortEnabled)
                return false;
        ...
        private bool LongEntry(int signal)
        {
            ...
            if (Position.IsNeutral && !IsLongEnabled)
                return false;
        ...
        private double kAtrStop19 = 0d;
        private double kAtrStop25 = 0d;
        ...
        protected override bool IsPosPositive
             => Position != null &&
                 (
                     ...
                     (Position.IsLong && XTrend.IsDown && ((double)Position.Price1).IsLessThan(XTrend.Ma + KAtrStop * VolatilityUnit2)) ||
                     (Position.IsShort && XTrend.IsDown && ((double)Position.Price1).IsGreaterThan(XTrend.Ma - KAtrStop * VolatilityUnit2)) ||
                     (Position.IsShort && XTrend.IsUp && ((double)Position.Price1).IsGreaterThan(XTrend.Ma - KAtrStop * VolatilityUnit2))
                  );
        protected bool IsProfitTooMuch => Position != null && KAtrStop1.IsGreaterThan(0) &&
                 (
            (Position.IsLong && ((double)Position.Price1).IsLessThan(XTrend.Ma - KAtrStop1 * VolatilityUnit2)) ||
            (Position.IsShort && ((double)Position.Price1).IsGreaterThan(XTrend.Ma + KAtrStop1 * VolatilityUnit2))
            );
        protected bool IsLossTooMuch => Position != null && KAtrStop2.IsGreaterThan(0) &&
                 (
            (Position.IsLong && ((double)Position.Price1).IsGreaterThan(XTrend.Ma + KAtrStop2 * VolatilityUnit2)) ||
            (Position.IsShort && ((double)Position.Price1).IsLessThan(XTrend.Ma - KAtrStop2 * VolatilityUnit2))
            );
                ...
                else if (Position.Quantity >= Contracts)
                {
                    ...
                    if (IsPosPositive)
                    {
                        Mode = IsMode5Enabled && IsPosAbsMaxReached
                            ? 5
                            : 3;
                             // 27.09.2017 (IsProfitTooMuch ? 8 : 3);
                        // :  Mode = 3;
                        if (IsMode3Delayed && Mode == 8 && prevmode != 8)
                            SkipTheTick(0);
                        //if (RandMode > 0 && TrEntryEnabled.Value)
                        //{
                        //    AskRandom();
                        //    Mode = RandAnswer ? Mode : 10;
                        //    if (!RandAnswer)
                        //        SkipTheTick(0);
                        //}
                    }
                    ...
                    {
                        // 27.09.2017 Mode = IsLossTooMuch ? 8 : 4;
                        Mode = 4;
                        if (IsMode2Delayed && Mode == 8 && prevmode != 8)
                            SkipTheTick(0);
                        //if (RandMode > 0 && TrEntryEnabled.Value)
                        //{
                        //    AskRandom();
