...
namespace GS.Trade.Strategies
{
    public class Z00721 : Z007
    {
        ...
        private void ChangeMode123()
        {
            ...
            if (PosPriceMode == 1)
            {
                posPrice = Position.Price1;
            }
            else
            {
                posPrice = Position.Price3.IsEquals(0m) ? Position.Price1 : Position.Price3;
            }
            ...
            if (Position.IsOpened )
            {
                if (Position.Quantity < Contracts)
                {
                    ...
                    {
                        if ( // TrendMode == 1 &&
                            (Position.IsLong && XTrend.Ma > (double) posPrice) ||
                            (Position.IsShort && XTrend.Ma < (double) posPrice)
                           )
                            Mode = _maxContracts ? 3 : 2;
                        else
                            Mode = _maxContracts ? 4 : 2;
                    }
                }
