        ...
        protected override void CalcPosMinMax()
        {
            ...
            if (_swingCountEntry >= 2)
            {
                if (Position.IsShort)
                {
                    ...
                    //  IsPosMinReached = !PositionMin.HasValue || XTrend.Ma.IsLessThan(PositionMin.GetValidValue);
                ...
                else if (Position.IsLong)
                {
                    ...
                    //09.04.2018
                    //   IsPosMaxReached = !PositionMax.HasValue || XTrend.Ma.IsGreaterThan(PositionMax.GetValidValue);
        ...
        private void ChangeMode123()
        {
            ...
            if (Position.IsOpened )
            {
                ...
                else if (Position.Quantity >= Contracts)
                {
                    ...
                    if (
                        // Updated next string 15.09.29
                        // (Mode == 3 || Mode == 4) &&
                        (Position.IsLong && XTrend.Ma > (double)Position.Price1) ||
                        (Position.IsShort && XTrend.Ma < (double)Position.Price1)
                        // (Position.IsLong && XTrend.High > (double)Position.Price1) ||
                        //(Position.IsShort && XTrend.Low < (double)Position.Price1)
                        //(Position.IsLong && XTrend.Low > (double)Position.Price1) ||
                        //(Position.IsShort && XTrend.High < (double)Position.Price1)
                        )
                    {
                        //Mode = (Position.IsLong && IsPosMaxReached) || (Position.IsShort && IsPosMinReached)
