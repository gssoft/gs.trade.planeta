        ...
        private void ChangeMode123()
        {
            ...
            if (Position.IsOpened )
            {
                ...
                else
                {
                    ...
                    //    return;
                    //}
                    if (KAtrStop.IsEquals(0)) return;
                    if (
                        (IsLong && (XTrend.Ma - LastTradeMaValue).IsGreaterThan(2*Volatility*KAtrStop)) ||
                        (IsShort && (-XTrend.Ma + LastTradeMaValue).IsGreaterThan(2*Volatility*KAtrStop))
                       )
                    { Mode = 8;}
                }
            }
