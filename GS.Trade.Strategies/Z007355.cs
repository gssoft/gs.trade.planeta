            ...
            if (Position.IsOpened )
            {
                if (Position.Quantity < Contracts)
                {
                    ...
                    //{
                        ...
                        if ( // TrendMode == 1 &&
                            (Position.IsLong && XTrend.Ma.IsGreaterThan(TrailingTrLow.GetValidValue)) ||
                            (Position.IsShort && XTrend.Ma.IsLessThan(TrailingTrHigh.GetValidValue))
                            )
                            Mode = 2;
                        else
                        {
