            ...
            if (Position.IsOpened )
            {
                ...
                else if (Position.Quantity >= Contracts)
                {
                    ...
                    else
                    {
                        ...
                        {
                            if (
                                (Position.IsLong && XTrend.IsUp && XTrend.High.IsLessThan(PositionStop.GetValidValue)) ||
                                (Position.IsShort && XTrend.IsDown && XTrend.Low.IsGreaterThan(PositionStop.GetValidValue))
                                )
                                Mode = 4;
                            else
                                Mode = 8;
