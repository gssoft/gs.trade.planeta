...
namespace GS.Trade.Strategies
{
    public class Z0073523A2 : Z007
    {
        ...
        protected override int Entry()
        {
            ...
            //if (Position.IsNeutral && LastSignalActivated == 19) // && _swingCountEntry < 1)
            if (IsNeutral && LastSignalActivated == 19)
            {
                if (Math.Abs(LastTradeMaValue - XTrend.Ma).IsLessThan(VolatilityUnit2 * 1d))
                    return 0;
            }
        ...
        private double kAtrStop19 = 2d;
        private double kAtrStop25 = -3d;
