    ...
    public class Z00515 : Z005
    {
        ...
        private bool LongEntry(int dir)
        {
            ...
            if ((TakeLong(entrySignal) ||
              //  (dir < 0 && PositionStop.IsLessThan((float)XTrend.Ma) && TakeLong(entrySignal2)) ||
        ...
        private new bool ShortEntry()
        {
            ...
            if ((TakeShort(entrySignal) /*|| ( IsSafeContracts &&  TakeShort(entrySignal2)) */) && _swingCountEntry >= swCntEntry  // &&
                //  (
                //   (Position.IsShort && (XTrend.Ma.CompareTo((double)Position.LastTradeSellPrice) > 0 || LastPositionChanged == PositionChangedEnum.ReSizedDown)) ||
                //   (Position.IsLong || Position.IsNeutral)
                //  )
        ...
        private new bool LongEntry()
        {
            ...
            if ((TakeLong(entrySignal) /*|| (IsSafeContracts && TakeLong(entrySignal2))*/) && _swingCountEntry >= swCntEntry // &&
                //  (
                //  (Position.IsLong &&
                //   (XTrend.Ma.CompareTo((double)Position.LastTradeBuyPrice) < 0 || LastPositionChanged == PositionChangedEnum.ReSizedDown)) ||
                //   (Position.IsShort || Position.IsNeutral)
