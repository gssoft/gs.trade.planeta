...
namespace GS.Trade.TradeTerminals64.Quik
{
    public sealed partial class QuikTradeTerminal
    {
        ...
        public class TransactionReplyItem64 //: IProcessingAction<TransactionReplyItem>
        {
            public ulong TransId { get; set; }
            public ulong OrderNum { get; set; }
            public T2QResults64 Result { get; set; }
            public long ExtendedErrorCode { get; set; }
            public long ReplyCode { get; set; }
            public string ReplyMessage { get; set; }
            public DateTime ReplyDateTime { get; set; }
            public override string ToString()
            {
        ...
        //}
        // x64
        //private void SendTransactionReplyEventArgs64(
        //    TransactionReplyItem64 transactionReply, Action<IEventArgs1> action)
        //{
        //    var eargs = new EventArgs1
        //    {
        //        Process = "QuikTransactionsProcess",
        ...
        //        action(eargs);
        //}
        private void SendTransactionReplyEventArgs<T>(
            T transactionReply, Action<IEventArgs1> action)
        {
            var eargs = new EventArgs1
            {
                Process = "QuikTransactionsProcess",
           ...
           UInt32 dwTransId, Double dOrderNum, [MarshalAs(UnmanagedType.LPStr)] string replyMessage)
        {
            ...
            SendTransactionReplyEventArgs<TransactionReplyItem>(tri, TransactionReplyResolve);
            SendTransactionReplyEventArgs<TransactionReplyItem>(tri, TransactionReplyItemTracking);
        }
        // x64
        public void TransactionReply64(long nResult, long nExtendedErrorCode, long nReplyCode,
            ulong dwTransId, ulong dOrderNum, [MarshalAs(UnmanagedType.LPStr)] string replyMessage)
        {
            var tri = new TransactionReplyItem64
            {
                TransId = dwTransId,
                OrderNum = dOrderNum,
                Result = (T2QResults64)nResult,
                ExtendedErrorCode = nExtendedErrorCode,
                ReplyCode = nReplyCode,
                ReplyMessage = replyMessage,
                ReplyDateTime = DateTime.Now
            };
            SendTransactionReplyEventArgs<TransactionReplyItem64>(tri, TransactionReplyResolve);
            SendTransactionReplyEventArgs<TransactionReplyItem64>(tri, TransactionReplyItemTracking);
        }
        public void TransactionReplyResolve(IEventArgs1 args)
        {
            ...
            var t = Transactions.GetByKey((ulong)dwTransId);
            if (t == null)
            {
               ...
               )
            {
                ...
                switch (nReplyCode)
                {
                    ...
                    default:
                        ...
                        t.Order.ShortInfo, $"{method} {OrderErrorMsg.UnknownReplyCode}: {t.Order.TrReplyCode}",
                ...
                Evlm2(orderClone.Number == 0 ? EvlResult.FATAL : EvlResult.SUCCESS, EvlSubject.TECHNOLOGY,
                        t.StrategyStr, orderClone.ShortInfo, $"{method}", replyMessage,
                        t.ShortDescription);
            ...
            //t.Action == QuikTransactionActionEnum.KillStopLimit))
            {
                ...
                Evlm2(EvlResult.SUCCESS, EvlSubject.TECHNOLOGY, t.StrategyStr,
                    orderClone.ShortInfo, $"{method}", replyMessage, t.ShortDescription);
