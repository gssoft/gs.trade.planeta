...
namespace GS.Trade.TradeTerminals64
{
    public class TradeTerminals : Element1<string>, ITradeTerminals
    {
        ...
        public override string Key => FullName + "@" + (Code.HasValue() ? Code : "Terminals");
        ...
        // for TradeContext2
        //public ITradeTerminal64 RegisterTradeTerminal(string type, string key,
        ...
        //            break;
        //        case "SIMULATOR":
        //            tt = new SimulateTerminal
        //                (type, key, orders, trades, tx);
        //            break;
        //    }
        ...
        public ITradeTerminal RegisterTradeTerminal(string type, string key,
            IOrders orders, IOrders orders2, ITrades trades, IEventLog evl, ITradeContext tx)
        {
            var upKey = (type.Trim() + key.Trim()).ToUpper();
            if (TradeTerminalCollection.TryGetValue(upKey, out var tt)) return tt;
        ...
        {
            foreach (var value in TradeTerminalCollection.Values)
            {
                if (value is IHaveQueue t)
                {
                    t.DeQueueProcess();
                }
            }
        ...
        public void OrderResolveProcess()
        {
            foreach (var value in TradeTerminalCollection.Values)
            {
                if (value is INeedOrderResolve t)
                {
                    t.OrderResolveProcess();
                }
            }
        }
