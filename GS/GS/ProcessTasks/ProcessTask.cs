...
{
    public class ProcessTask<TInput> : Element35<string, TInput, IEventArgs1>, IDisposable
    {
        ...
        public string WorkTaskStatusStr =>
           ...
           $"Changed: {LastStatusChangedDateTime:HH:mm:ss}, " +
           $"Elapsed: {LastStatusChangedElapsed:hh\\:mm\\:ss\\.fff}";
        ...
        public void ItemsProcessingDefault(IEnumerable<TInput> items)
        {
        ...
        public override void DeQueueProcess()
        {
            ...
            //    Queue.Clear();
            //    Evlm1(EvlResult.FATAL, EvlSubject.TECHNOLOGY, ParentTypeName, TypeName,
            //            m, "ItemsProcessingAction IS NULL, Entering Disabled Mode, Good Bye", ToString());
            //    return;
            //}         
            while (!Queue.IsEmpty)
            {
                try
                {
                    ...
                    ItemsProcessingAction.Invoke(items);
                }
                catch (Exception e)
                {
                    IsEnabled = false;
                    SendException(e);
                    Evlm1(EvlResult.FATAL, EvlSubject.TECHNOLOGY, ParentAndMyTypeName, e.GetType().ToString(),
        ...
        public void Start()
        {
            ...
            try
            {
                ...
                if (ItemsProcessingAction == null && ItemProcessingAction == null)
                    throw new NullReferenceException("ItemsProcessingAction == null && ItemProcessingAction == null");
                if (ItemsProcessingAction == null && ItemProcessingAction != null)
                    ItemsProcessingAction = ItemsProcessingDefault;

                Evlm2(EvlResult.INFO, EvlSubject.INIT, ParentTypeName, TypeName, $"{m} Begin", AllStatusStr, ToString());
                ...
                Evlm2(EvlResult.SUCCESS, EvlSubject.INIT, ParentTypeName, TypeName, $"{m} Finish", AllStatusStr, ToString());
            }
            catch (Exception e)
            {
                Evlm2(EvlResult.FATAL, EvlSubject.TECHNOLOGY, ParentTypeName, e.GetType().ToString(),
                        m, e.Message, ToString());
                SendException(e);
            }
