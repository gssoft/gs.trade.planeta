...
namespace GS.WorkTasks
{
    ...
    public class WorkTask3 : Element1<string>, IWorkTask3, IDisposable
    {
        #region Events
        ...
        public string TaskStatusStr =>
            Task != null
                ? $"TaskStatus: {Task.Status.ToString()}, " + $"IsCanceled: {Task.IsCanceled}, " +
                  $"IsCompleted: {Task.IsCompleted}, " + $"IsFaulted: {Task.IsFaulted}"
                : "Task is Null";

        public bool IsUpToDate => DateTime.Now - LastTryWorkDateTime < TimeSpan.FromSeconds(2 * TimeInterval);
        ...
        private bool CreateTask()
        {
            ...
            Task = new Task(() =>
            {
                ...
                Evlm2(EvlResult.SUCCESS, EvlSubject.TECHNOLOGY, FullCode, Code, "Start()", TaskStatusStr,"");
