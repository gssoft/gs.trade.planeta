...
namespace GS.Trade.Storage2
{
    ...
    //public class TradeStorage21  : GS.Storages.Storage<string, ITradeStorage>, ITradeStorage //, IHaveQueue
    public class TradeStorage21 : Element21<string, ITradeStorage, Containers5.DictionaryContainer<string, ITradeStorage>>, ITradeStorage
    {
        ...
        {
            base.Init(evl);
            _primary = Collection.Items.FirstOrDefault(trs => trs.IsEnabled && trs.IsPrimary);
            if (_primary == null)
                SendExceptionMessage3(Name, "Init().SetPrimary", "Primary TradeStorage is not Found", Code,
                                                new NullReferenceException("Primary TradeStorage is not Found"));
            else
                Evlm2(EvlResult.SUCCESS, EvlSubject.TECHNOLOGY, Name, _primary.Name, "Set Primary", "Primary=" + _primary.Name, _primary.ToString());

            Evlm2(EvlResult.SUCCESS, EvlSubject.TECHNOLOGY, FullName, Code, "Init()", GetType().FullName, "");
        ...
        public IEnumerable<ProcessManager2.ProcessProcedure> DeQueueProcesses
        {
            get
            {
                ...
                foreach (var trs in Collection.Items.Where(t => t.IsEnabled))
                {
        ...
        public IStrategy Register(IStrategy s)
        {
            try
            {
                if (s == null)
                {
                    SendExceptionMessage3(Name, "TradeStorage.Register(IStrategy s==Null)", "Null Reference", Code,
                        new Exception());
        ...
        public ITicker Register(ITicker t)
        {
            ...
            foreach (var trs in Collection.Items.Where(ti => ti.IsEnabled))
            {
        ...
        {
            foreach (var trs in Collection.Items.Where(t => t.IsEnabled && t.IsQueueEnabled))
            {
                trs.DeQueueProcess();
            }
        }
