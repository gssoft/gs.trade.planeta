...
namespace TcpIpSockets.TcpServer04
{
                    ...
                    DictionaryContainer<string, ITcpClientHandler>, ITcpClientHandler>, ITcpServer
    {
        ...
        private void TcpListenerMainAction(CancellationToken canceltoken)
        {
            ...
            ConsoleSync.WriteLineT($"{m} {Key} {TcpListener.LocalEndpoint} Stopped");
        }
        ...
        public void Subscribe(string topicKey, EventHandler<IEventArgs> callback)
        {
            var m = MethodBase.GetCurrentMethod().Name + "()";
            try
            {
        ...
        public void Subscribe(string topicKey, EventHandler<List<string>> callback)
        {
            ...
            try
            {
                EventHubT1?.Subscribe(topicKey, callback);
                Evlm2(EvlResult.SUCCESS, EvlSubject.INIT, ParentTypeName, TypeName,
                    $"{m}", $"Topic: {topicKey}", ToString());
            }
            catch (Exception e)
        ...
        public void Subscribe(string topicKey, EventHandler<IEventArgs> callback, ITcpClientHandler tcpHandler)
        {
            var m = MethodBase.GetCurrentMethod().Name + "()";
            try
            {
