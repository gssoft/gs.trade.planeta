...
namespace TcpIpSockets.TcpClientHandler03T2
{
    ...
    {      
        ...
        public void SendMessage(IHaveContent<TContent> message)
        {
            ProcessTask.EnQueue(message);
        }
        public bool SendMessage(params string[] strarr)
        {
            ...
            try
            {
                ...
                {
                    var v = new MessageStrArr(strarr) as IHaveContent<TContent>;
                    ProcessTask.EnQueue(v);
                }             
                else if(typeof(TContent) == typeof(List<string>))
                {
                    var v = new MessageLstStr(strarr) as IHaveContent<TContent>;
                    ProcessTask.EnQueue(v);
                }
