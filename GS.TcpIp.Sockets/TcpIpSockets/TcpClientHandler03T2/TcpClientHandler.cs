...
{    
    public partial class TcpClientHandler<TMessage, TContent> : Element1<string>,
        ITcpClientHandler<TMessage, TContent>
        where TMessage : IHaveIndex<string>, IHaveContent<TContent>
    {
        ...
        public ITcpServer<TMessage> TcpServer;
        ...
        private void TcpNetworkStreamReadProcess(CancellationToken canceltoken)
        {
            ...
            while (_working)
            {
                try
                {
                    ...
                    var dto = BinarySerialization.DeSerialize<TContent>(dtoBytes);

                    if (typeof (TContent) == typeof (string[]))
                    {
                        var v = new MessageStrArr(dto as string[]);
                        if (IsTcpServer)
                            TcpServerMessageRecievedProcessing(v);
                        else
                            TcpClientMessageRecievedProcessing(v);
                    }
                    else if(typeof(TContent) == typeof(List<string>))
                    {
                        var v = new MessageLstStr(dto as List<string>);
                        if (IsTcpServer)
                            TcpServerMessageRecievedProcessing(v);
                        else
                            TcpClientMessageRecievedProcessing(v);
        ...
        private void Register(TcpClientHandler<TMessage, TContent> tcpClientHandler)
        {
            var m = MethodBase.GetCurrentMethod().Name + "()";
