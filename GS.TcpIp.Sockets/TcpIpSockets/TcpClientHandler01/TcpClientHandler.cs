...
{    
    public partial class TcpClientHandler: ElementPrTsk01<string[]>, ITcpClientHandler
    {
        ...
        // public DateTime LasInteractiontDateTime { get; private set; }
        ...
        public void Start()
        {
            ...
            MainActionTask.Start(TcpReadNetworkStreamProcess);
        }
        public void Stop()
        {
            var m = MethodBase.GetCurrentMethod().Name + "()";
...
//            Thread.Sleep(1000);
        ...
        private void TcpReadNetworkStreamProcess(CancellationToken canceltoken)
        {
            var m = MethodBase.GetCurrentMethod().Name + "()";
            Evlm2(EvlResult.INFO, EvlSubject.TECHNOLOGY, ParentTypeName, TypeName, m,
                             $"{Key} Start NetworkStream.Read Process", ToString());
            _working = true;
            while (_working)
            {
                try
                {
                    ...
                    if (IsTcpServer)
                        ServerMessageRecievedProcessing(dto);
                    else
                        ClientMessageRecievedProcessing(dto);
        ...
        public void CloseSocket1()
        {
            ...
            try
            {
                if (ConnectionStatus == ConnectionEnum.CloseRequestAccepted)
                {
                    ConsoleSync.WriteLineT($"{m}: {TcpClientKey} SocketConnected:{ClientSocket.Connected} ConnStatus:{ConnectionStatus} Already in Use");
                    return;
                }
                ConnectionStatus = ConnectionEnum.CloseRequestAccepted;

                //               SetReadWorkingStatus(false);
                //               CancelTokenSource.Cancel();

                // ClientSocket.Close();
                // OnSocketClosed(TcpClientKey);
                int seconds = 15;
                //WaitToCompleteTask = Task.Factory
                //    .StartNew(p=>WaitToCompleteTaskAction(seconds),seconds);

                Task.Factory.StartNew(p => Task?.WaitingForTaskCompleting(seconds),seconds);

                //WaitToCompleteTask = Task.Factory.StartNew(async () =>
                //{
                //    try
                //    {
                //        await WaitToCompleteActionAsync().WithTimeout(TimeSpan.FromSeconds(15));
                //    }
                ...
                ConsoleSync.WriteLineT($"{m}: {TcpClientKey} SocketConnected:{ClientSocket.Connected} ConnStatus:{ConnectionStatus}");
            }
            catch (Exception e)
            {
                e.PrintExceptions(this,m);
