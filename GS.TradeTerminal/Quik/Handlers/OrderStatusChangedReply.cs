...
namespace GS.Trade.TradeTerminals.Quik
{
    public sealed partial class QuikTradeTerminal
    {
        ...
        public void OrderStatusReplyProcessing(IEventArgs args)
        {
            ...
            var orderStatusReply = args.Object as OrderStatusReply;
            if (orderStatusReply == null)
            {
                Evlm2(EvlResult.FATAL, EvlSubject.PROGRAMMING, ParentName, Name,
                        methodname,
                        $"Args IS NOT {typeof(OrderStatusReply).Name}", "Sorry");
                return;
            }
            ...
            if (ord != null)
            {
                ...
                Evlm2(EvlResult.SUCCESS, EvlSubject.TRADING,
                    ord.StrategyTimeIntTickerString, ord.ShortInfo + $" {area}", $"{methodname} {ord.ShortDescription}", ord.ToString(), Code);
            ...
            else
            {
                ...
                if (ord == null)
                {
                    ...
                    Evlm2(EvlResult.SUCCESS, EvlSubject.TRADING,
                        o.StrategyTimeIntTickerString, o.ShortInfo + $" {area}", $"{methodname} {o.ShortDescription}", o.ToString(), Code);
                ...
                else // Already Filled
                {
                    ...
                    FireChangedEventToStrategy(ord.Clone(), "ORDERS", "ORDER.STATUSCHANGED", "COMPLETED");
                    Evlm2(EvlResult.SUCCESS, EvlSubject.TRADING, ord.StrategyTimeIntTickerString,
                                            ord.ShortInfo + $" {area}", $"{methodname} {ord.ShortDescription}",
                                            ord.ToString(), Code);
                    if (ord.Strategy != null)
                        _tx.TradeStorage?.SaveChanges(StorageOperationEnum.AddOrUpdate, ord);
