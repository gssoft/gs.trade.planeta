...
namespace GS.Trade.TradeTerminals.Quik
{
    public sealed partial class QuikTradeTerminal
    {
        ...
        private TradeOperationEnum ConvertSideToEnum(int isSell)
        {
            switch (isSell)
            {
            ...
            string sClientCode, double dOrderNumber, double dCommissionTs)
        {
            if (IsProcessTaskInUse)
            {
                ...
                //    Operation = "AddOrUpdate",
                //    Object = tradeReply,
                //    ProcessingAction = NewTradeReplyProcessing
                //};
                //ProcessTask?.EnQueue(eargs);
        ...
        //double commissionTs)
        {
            ...
            if (tradeReply == null)
            {
                Evlm2(EvlResult.FATAL, EvlSubject.TRADING, Code, "TradeReply",
                        methodname, $"Args is not {typeof(NewTradeReply).Name}", "Sorry. See you Later");
                return;
            }
            ...
            if (!_checkMaxTradeNumber.SetIfGreaterThanMe(account, number))
            {
                Evlm2(EvlResult.WARNING, EvlSubject.TRADING, Code, tradeReply.ShortInfo(),
                    $"{methodname} TradeNumber:[{number}] IsLessOrEquals Than Max",
            ...
            if (classCode == "FUTEVN")
            {
                Evlm2(EvlResult.WARNING, EvlSubject.TRADING, DllNamePath2QuikPair, t.ShortInfo,
                    $"{methodname} EveningSession Trade Ignore",
                    t.ShortDescription, t.ToString());
                return;
            }
            Evlm2(EvlResult.SUCCESS, EvlSubject.TRADING, DllNamePath2QuikPair,  t.ShortInfo, methodname,
                    t.ShortDescription, t.ToString());
